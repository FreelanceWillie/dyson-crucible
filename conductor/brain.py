...
FAQ = (
    ...
    "- More like this: reseed a fresh batch from a winner.\n"
    "- Animate (an entry point on the home screen): two ways to bring a hero to life, "
    "both local. (1) Pose frames: pick one of your picked heroes, choose poses (idle, "
    "walk, run, attack, cast, cheer), and it renders that same character in each pose "
    "(sprite frames). It keeps identity with IP-Adapter and controls the pose with "
    "ControlNet. Fits a 4GB GPU. (2) Idle loop: describe an ambient motion (breathing, "
    "cape sway) and it makes a short looping GIF with AnimateDiff. On a 4GB GPU idle "
    "loops exceed VRAM and page to system RAM, so they work but run slow; keep frames "
    "low and size 384.\n"
    "- Feature packs (in Settings): optional extras download on demand. 'pose' powers "
